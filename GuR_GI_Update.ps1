...
If(!(test-path "$PSScriptRoot\upgraded-meme\")) {
    ...
    & "$PSScriptRoot\upgraded-meme\git\cmd\git.exe" fetch origin main
    & "$PSScriptRoot\upgraded-meme\git\cmd\git.exe" reset --hard
    & "$PSScriptRoot\upgraded-meme\git\cmd\git.exe" pull
}
